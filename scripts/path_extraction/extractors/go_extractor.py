"""
GO hierarchical path extractor.
Enhanced version with comprehensive error handling, caching, statistics, and logging.
Uses GO OBO file for hierarchical path extraction with goatools.
"""
...
class GOExtractor(BaseExtractor):
    """Enhanced GO extractor with comprehensive error handling and optimization."""
    ...
    def __init__(self, config: Dict[str, Any], vocab_name: str):
        ...
        self.max_path_length = 200  # Reasonable depth for GO hierarchies
        self.max_paths_per_code = 2000  # Same as SNOMED limit
        
        # Load GO data with retry logic (only if not in multiprocessing context)
        if not hasattr(self, '_skip_initial_load'):
            self._load_go_data_with_retry()
        
        self.logger.info(f"Initialized GO extractor for {vocab_name}")
    ...
    def validate_code(self, code: str) -> bool:
        """
        Validate GO code format.
        GO codes follow the pattern GO:0000000 (GO: followed by 7 digits)
        """
        ...
        # GO pattern: GO:0000000 (GO: followed by exactly 7 digits)
        go_pattern = r'^GO:\d{7}$'
        
        return bool(re.match(go_pattern, code))
        
    def extract_paths(self, code: str, tty: Optional[str] = None, cui: Optional[str] = None) -> List[List[Dict[str, str]]]:
        """
        Extract hierarchical paths for a GO code with enhanced error handling.
        
        Args:
            code: GO code (e.g., "GO:0006915")
            tty: Not used for GO
        ...
        if not self.validate_code(code):
            self.logger.warning(f"Invalid GO code format: {code}")
            if cui:
        ...
        try:
            ...
            self.failed_codes_cache.add(code)
            self.logger.error(f"Error extracting GO paths for {code}: {str(e)}")
    ...
    def _find_paths_with_retry(self, code: str) -> List[List[Dict[str, str]]]:
        """Find GO paths with retry logic for file operations."""
    ...
    def _find_paths(self, code: str) -> List[List[Dict[str, str]]]:
        """
        Find hierarchical paths for a GO code.
        Returns paths in ROOT->LEAF format (consistent with other vocabularies).
        """
        # Lazy load GO data if not already loaded (for multiprocessing)
        if not self.go_dag:
            self._load_go_data_with_retry()
        
        # Use the GO.py logic adapted for our format
        if code not in self.go_dag:
            return []
        
        term = self.go_dag[code]
        ...
        # This matches the corrected HPO implementation
    ...
    def get_comprehensive_statistics(self) -> Dict[str, Any]:
        ...
        enhanced_stats = {
            ...
            **self.stats,
            ...
            'total_inactive_codes': 0,  # GO doesn't have inactive codes like SNOMED
    ...
    def _load_go_data_with_retry(self):
        """Load GO data with retry logic."""
        
        for attempt in range(self.max_retries):
            try:
                self.logger.info(f"GO data load attempt {attempt + 1}/{self.max_retries}")
                self._load_go_data()
                return
                ...
                if attempt < self.max_retries - 1:
                    delay = min(self.base_retry_delay * (2 ** attempt), self.max_retry_delay)
                    time.sleep(delay)
                    self.logger.warning(f"GO data load retry {attempt + 1} failed, retrying in {delay}s: {str(e)}")
                    continue
                else:
                    raise
                    
        # If we get here, all retries failed
        raise Exception(f"All {self.max_retries} GO data load attempts failed")
    ...
    def _load_go_data(self):
        ...
        try:
            ...
            # GO OBO file path
            from pathlib import Path
            go_obo_path = str((Path(__file__).parents[3] / "path_data" / "go-basic.obo").resolve())
            
            # Load GO DAG
            self.logger.info(f"Loading GO OBO from {go_obo_path}")
            self.go_dag = GODag(go_obo_path)
            
            self.logger.info(f"Loaded {len(self.go_dag)} GO terms")
            
        except Exception as e:
            self.logger.error(f"Error loading GO data: {e}")
            raise
